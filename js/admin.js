              ...
              <tr data-id="${l.id}">
                ...
                <td>
                  <div class="row-actions">
                      ...
                      <button type="button" onclick='openQuizModal("${l.id}", "${U.escapeHtml(l.title)}")'><i class="ph ph-exam"></i> Manage Quiz</button>
...
function openModuleForm(m) {
    ...
    const coverEl = document.getElementById('moduleCoverImage'); if (coverEl) coverEl.value = m?.cover_image_url || '';
...
async function handleModuleSubmit(e) {
    ...
    const payload = {
        ...
        cover_image_url: document.getElementById('moduleCoverImage')?.value || null,
...
async function openQuizModal(lessonId, lessonTitle) {
    document.getElementById('quizFormError').classList.remove('visible');
    document.getElementById('quizSettingsForm').reset();
    ...
    // fetch quiz
    const { data: qData, error: qErr } = await window.CatalystDB
        .from('lesson_quizzes')
        .select('*')
    ...
    if (qData) {
        document.getElementById('quizId').value = qData.id;
        document.getElementById('quizTitle').value = qData.title || '';
        document.getElementById('quizPassingPct').value = qData.passing_percentage || 80;
        document.getElementById('quizPoints').value = qData.points_value || 10;
        document.getElementById('quizInstructions').value = qData.instructions || '';
        document.getElementById('quizEnabled').checked = !!qData.enabled;
        
        document.getElementById('quizCreationHint').style.display = 'none';
        document.getElementById('quizQuestionsArea').style.display = 'block';
        await loadQuizQuestions(qData.id);
    } else {
        document.getElementById('quizTitle').value = lessonTitle + " - Quiz";
        document.getElementById('quizCreationHint').style.display = 'block';
    }
    
    U.openModal('quizModal');
}

// Add event listener manually since we injected it
document.getElementById('quizSettingsForm')?.addEventListener('submit', async (e) => {
    ...
    const payload = {
        ...
        passing_percentage: parseInt(document.getElementById('quizPassingPct').value, 10) || 80,
        points_value: parseInt(document.getElementById('quizPoints').value, 10) || 0,
    ...
    let res;
    if (quizId) {
        res = await window.CatalystDB.from('lesson_quizzes').update(payload).eq('id', quizId);
    } else {
        res = await window.CatalystDB.from('lesson_quizzes').insert([payload]).select();
    }
    ...
    if (res.error) {
        ...
        U.toast('Quiz settings saved', 'success');
        if (!quizId && res.data && res.data.length > 0) {
            document.getElementById('quizId').value = res.data[0].id;
            document.getElementById('quizCreationHint').style.display = 'none';
            document.getElementById('quizQuestionsArea').style.display = 'block';
            await loadQuizQuestions(res.data[0].id);
        }
    }
});
...
async function loadQuizQuestions(quizId) {
    ...
    list.innerHTML = '<div class="loading-state"><i class="ph ph-spinner-gap ph-spin"></i> Loading questions...</div>';
    
    const [{ data: qs, error: qsErr }, { data: opts }] = await Promise.all([
        window.CatalystDB.from('quiz_questions').select('*').eq('quiz_id', quizId).order('sort_order'),
        window.CatalystDB.from('quiz_options').select('*').order('sort_order')
    ]);
    
    if (qsErr) {
        list.innerHTML = `<div class="error-state">Error loading questions: ${qsErr.message}</div>`;
        return;
    }
    ...
    if (currentQuestions.length === 0) {
        list.innerHTML = `<div class="empty-state">No questions added yet.</div>`;
        return;
    }
    
    let html = '';
    currentQuestions.forEach(q => {
        const qOpts = currentOptions.filter(o => o.question_id === q.id);
        const correctCount = qOpts.filter(o => o.is_correct).length;
        html += `
            <div style="border:1px solid var(--border-color); border-radius:var(--radius-sm); padding:15px; margin-bottom:15px; position:relative;">
                <div style="font-weight:600; margin-bottom:10px;">${U.escapeHtml(q.question_text).substring(0, 60)}...</div>
                <div style="font-size:0.85rem; color:var(--text-muted); margin-bottom:10px;">
                    Order: ${q.sort_order} | Points: ${q.points || 0} | Options: ${qOpts.length} (${correctCount} correct) | 
                    ${q.published ? '<span style="color:var(--success-color);">Published</span>' : '<span style="color:var(--danger-color);">Draft</span>'}
                </div>
                <div>
                    <button type="button" class="btn-outline" onclick='openQuestionForm(${JSON.stringify(q).replace(/'/g, "&#39;")})'><i class="ph ph-pencil-simple"></i> Edit</button>
                    <button type="button" class="btn-outline danger" onclick="deleteQuestion('${q.id}')"><i class="ph ph-trash"></i> Drop</button>
                </div>
            </div>
        `;
    });
...
window.openQuestionForm = function(q = null) {
    document.getElementById('questionForm').reset();
    document.getElementById('questionId').value = q ? q.id : '';
    document.getElementById('questionText').value = q ? q.question_text : '';
    document.getElementById('questionOrder').value = q ? q.sort_order : 0;
    document.getElementById('questionPoints').value = q ? (q.points || 10) : 10;
    document.getElementById('questionFormTitle').textContent = q ? 'Edit Question' : 'Add Question';
    
    const errBox = document.getElementById('questionFormError');
    if (errBox) { errBox.style.display = 'none'; errBox.textContent = ''; }
    
    const optsArea = document.getElementById('optionsList');
    optsArea.innerHTML = '';
    
    if (q) {
        const qOpts = currentOptions.filter(o => o.question_id === q.id);
        qOpts.forEach(o => {
            appendOptionRow(o.id, o.option_text, o.is_correct, o.sort_order);
        });
    } else {
        appendOptionRow('', '', false, 0);
        appendOptionRow('', '', false, 1);
    }
    
    U.openModal('questionModal');
}

window.addOptionRow = function() {
    appendOptionRow('', '', false, document.querySelectorAll('.opt-row').length);
}

function appendOptionRow(id, text, isCorrect, sortOrder) {
    ...
    div.innerHTML = `
        ...
        <input type="radio" name="correct_option" class="opt-correct" ${isCorrect ? 'checked' : ''} title="Is Correct?">
        <input type="text" class="opt-text" value="${U.escapeHtml(text)}" required style="flex:1;" placeholder="Option text">
        <input type="number" class="opt-order" value="${sortOrder}" style="width:60px;" title="Sort Order">
        <button type="button" class="btn-outline danger" onclick="this.parentElement.remove()" style="padding:10px;"><i class="ph ph-trash"></i></button>
    `;
...
document.getElementById('questionForm')?.addEventListener('submit', async (e) => {
    ...
    const sortOrder = parseInt(document.getElementById('questionOrder').value, 10) || 0;
    const points = parseInt(document.getElementById('questionPoints').value, 10) || 0;
    
    const submitBtn = e.target.querySelector('button[type="submit"]');
    U.setLoading(submitBtn, true, 'Saving...');
    
    // Validation
    if (!questionText) return showError("Question text is required.");
    if (points < 0) return showError("Points must be >= 0.");
    
    const rows = document.querySelectorAll('.opt-row');
    if (rows.length < 2) return showError("At least 2 options are required.");
    ...
    for (const r of rows) {
        const oId = r.querySelector('.opt-id').value;
        const oCorrect = r.querySelector('.opt-correct').checked;
        const oText = r.querySelector('.opt-text').value.trim();
        const oOrder = parseInt(r.querySelector('.opt-order').value, 10) || 0;
        
        if (!oText) return showError("Every option must have text.");
        if (oCorrect) correctCount++;
        
        optPayloads.push({ id: oId || null, option_text: oText, is_correct: oCorrect, sort_order: oOrder });
    }
    
    if (correctCount !== 1) return showError(`Exactly 1 correct option is required. You selected ${correctCount}.`);
    ...
    let realQId = qId;
    if (qId) {
        const { error: updErr } = await window.CatalystDB.from('quiz_questions').update(qPayload).eq('id', qId);
        if (updErr) return showError(updErr.message);
    } else {
        const { data, error: insErr } = await window.CatalystDB.from('quiz_questions').insert([qPayload]).select();
        if (insErr) return showError(insErr.message);
        if (data && data.length > 0) realQId = data[0].id;
        else return showError("Failed to insert question.");
    }
    
    // Process options
    for (const op of optPayloads) {
        if (op.id) {
            optIdsToKeep.push(op.id);
            const { error } = await window.CatalystDB.from('quiz_options').update({
                option_text: op.option_text,
                is_correct: op.is_correct,
                sort_order: op.sort_order
            }).eq('id', op.id);
            if (error) return showError("Option update failed: " + error.message);
        } else {
            const { error } = await window.CatalystDB.from('quiz_options').insert([{
                question_id: realQId,
                option_text: op.option_text,
                is_correct: op.is_correct,
                sort_order: op.sort_order
            }]);
            if (error) return showError("Option insert failed: " + error.message);
        }
    }
    
    // Delete removed options
    if (qId) {
        const oldOpts = currentOptions.filter(o => o.question_id === qId);
        const toDelete = oldOpts.filter(o => !optIdsToKeep.includes(o.id)).map(o => o.id);
        for(let dId of toDelete) {
            const { error } = await window.CatalystDB.from('quiz_options').delete().eq('id', dId);
            if (error) return showError("Option delete failed: " + error.message);
        }
    }
    
    U.setLoading(submitBtn, false);
    U.closeModal('questionModal');
    U.toast('Question saved!', 'success');
    loadQuizQuestions(quizId);
});

window.deleteQuestion = async function(qId) {
    if (!confirm('Drop this question and all its options?')) return;
    const { error } = await window.CatalystDB.from('quiz_questions').delete().eq('id', qId);
    if (error) {
        U.toast(error.message, 'error');
        return;
    }
    loadQuizQuestions(document.getElementById('quizId').value);
}
